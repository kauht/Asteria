...
namespace features::chams {
    ...
    void* MakeMaterial(const char* name, const char* kv3_text) {
        ...
        materialsystem2::kv3_id_t id{ "generic", k_kv3Unk0, k_kv3Unk1 };

        if (!g_state.LoadKV3(buf.data() + 0x100, nullptr, kv3_text, &id, nullptr, 0)) return nullptr;

        void* handle{};
        g_state.CreateMaterial(g_state.material_system, &handle, name, buf.data() + 0x100, nullptr, 1);
