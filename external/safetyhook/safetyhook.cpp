...
Allocation::Allocation(std::shared_ptr<Allocator> allocator, uint8_t* address, size_t size) noexcept : m_allocator{std::move(allocator)}, m_address{address}, m_size{size} {}
...
std::expected<Allocation, Allocator::Error> Allocator::allocate_near(const std::vector<uint8_t*>& desired_addresses, size_t size, size_t max_distance) {
...
std::expected<Allocation, Allocator::Error> Allocator::internal_allocate_near(const std::vector<uint8_t*>& desired_addresses, size_t size, size_t max_distance) {
...
std::expected<uint8_t*, Allocator::Error> Allocator::allocate_nearby_memory(const std::vector<uint8_t*>& desired_addresses, size_t size, size_t max_distance) {
    ...
    for (auto p = desired_address; p > search_start && in_range(p, desired_addresses, max_distance); p = align_down(mbi.address - 1, si.allocation_granularity)) {
...
[[nodiscard]] static std::expected<void, InlineHook::Error> emit_jmp_ff(uint8_t* src, uint8_t* dst, uint8_t* data, size_t size = sizeof(JmpFF)) {
...
[[nodiscard]] static std::expected<void, InlineHook::Error> emit_jmp_e9(uint8_t* src, uint8_t* dst, size_t size = sizeof(JmpE9)) {
...
std::expected<InlineHook, InlineHook::Error> InlineHook::create(const std::shared_ptr<Allocator>& allocator, void* target, void* destination, Flags flags) {
    InlineHook hook{};

    if (const auto setup_result = hook.setup(allocator, reinterpret_cast<uint8_t*>(target), reinterpret_cast<uint8_t*>(destination)); !setup_result) {
...
std::expected<void, InlineHook::Error> InlineHook::setup(const std::shared_ptr<Allocator>& allocator, uint8_t* target, uint8_t* destination) {
    m_target = target;
...
std::expected<void, InlineHook::Error> InlineHook::e9_hook(const std::shared_ptr<Allocator>& allocator) {
    ...
    auto trampoline_epilogue = reinterpret_cast<TrampolineEpilogueE9*>(m_trampoline.address() + m_trampoline_size - sizeof(TrampolineEpilogueE9));
...
std::expected<void, InlineHook::Error> InlineHook::ff_hook(const std::shared_ptr<Allocator>& allocator) {
    ...
    const auto trampoline_epilogue = reinterpret_cast<TrampolineEpilogueFF*>(m_trampoline.data() + m_trampoline_size - sizeof(TrampolineEpilogueFF));
...
std::expected<void, InlineHook::Error> InlineHook::enable() {
    ...
    trap_threads(m_target, m_trampoline.data(), m_original_bytes.size(), [this, &error] {
        if (m_type == Type::E9) {
            ...
            if (auto result = emit_jmp_e9(m_target, reinterpret_cast<uint8_t*>(&trampoline_epilogue->jmp_to_destination), m_original_bytes.size()); !result) {
                error = result.error();
            }
        }
...
#if SAFETYHOOK_ARCH_X86_64
        if (m_type == Type::FF) {
            if (auto result = emit_jmp_ff(m_target, m_destination, m_target + sizeof(JmpFF), m_original_bytes.size()); !result) {
                error = result.error();
            }
        }
#endif
...
std::expected<void, InlineHook::Error> InlineHook::disable() {
    ...
    trap_threads(m_trampoline.data(), m_target, m_original_bytes.size(), [this] { std::copy(m_original_bytes.begin(), m_original_bytes.end(), m_target); });
...
constexpr std::array<uint8_t, 391> asm_data = {
    ...
    };
...
constexpr std::array<uint8_t, 391> asm_data = {
    ...
    ,
...
#endif
#elif SAFETYHOOK_ARCH_X86_32
constexpr std::array<uint8_t, 171> asm_data = {};
#endif
...
std::expected<MidHook, MidHook::Error> MidHook::create(const std::shared_ptr<Allocator>& allocator, void* target, MidHookFn destination, Flags flags) {
    MidHook hook{};
...
std::expected<void, MidHook::Error> MidHook::setup(const std::shared_ptr<Allocator>& allocator, uint8_t* target, MidHookFn destination_fn) {
    m_target = target;
...
std::expected<VmBasicInfo, OsError> vm_query(uint8_t* address) {
    ...
    unsigned long inode;
    char path[256];
    unsigned long last_end = reinterpret_cast<unsigned long>(system_info().min_address); // Track the end address of the last mapping.
    ...
    while (fgets(line, sizeof(line), maps) != nullptr) {
        ...
        if (last_end < start && addr >= last_end && addr < start) {
            info = std::make_optional<VmBasicInfo>({reinterpret_cast<uint8_t*>(last_end), start - last_end, VmAccess{}, true});
...
void trap_threads([[maybe_unused]] uint8_t* from, [[maybe_unused]] uint8_t* to, [[maybe_unused]] size_t len, const std::function<void()>& run_fn) {
...
bool vm_is_executable(uint8_t* address) {
    // Check if the address is in a valid module allowing us to potentially skip a
    // heavier memory query.
    HMODULE image{};
    if (!GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, reinterpret_cast<LPTSTR>(address), &image) || image == nullptr) {
        return vm_query(address).value_or(VmBasicInfo{}).access.execute;
    }
    ...
    for (auto i = 0; i < nt_hdr->FileHeader.NumberOfSections; ++i, ++section) {
        if (address >= image_base + section->VirtualAddress && address < image_base + section->VirtualAddress + section->Misc.VirtualSize) {
...
class TrapManager final {
  public:
    ...
    TrapInfo* find_trap(uint8_t* address) {
        auto search = std::find_if(m_traps.begin(), m_traps.end(), [address](auto& trap) { return address >= trap.second.from && address < trap.second.from + trap.second.len; });
    ...
    TrapInfo* find_trap_page(uint8_t* address) {
        auto search = std::find_if(m_traps.begin(), m_traps.end(), [address](auto& trap) { return address >= trap.second.from_page_start && address < trap.second.from_page_end; });

        if (search != m_traps.end()) {
            return &search->second;
        }

        search = std::find_if(m_traps.begin(), m_traps.end(), [address](auto& trap) { return address >= trap.second.to_page_start && address < trap.second.to_page_end; });
...
void trap_threads(uint8_t* from, uint8_t* to, size_t len, const std::function<void()>& run_fn) {
    ...
    // Make sure we aren't working on a different address in the same memory page
...
std::expected<VmtHook, VmtHook::Error> VmtHook::create(void* object) {
    ...
    // Count the number of virtual method pointers. We start at one to account for
    // the RTTI pointer.
